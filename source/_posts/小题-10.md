...
简单的解释一下这个问题：你有`M`个硬币，币值都不同，你还有一个总数`N`， 问凑成`N`的方式有多少种，其中 $ 1 \leq N \leq 250$，$ 1 \leq M \leq 50$。注意，同样的硬币组合的不同排列不算。比如你要凑成总数 5，你有币值为 1, 2, 3 的三种硬币。则答案为 5， 因为你有 5 种方式凑成总数 5，分别是`[1,1,1,1,1]`, `[1,1,1,2]`, `[1,1,3]`, `[2, 3]`, `[1, 2, 2]`。
